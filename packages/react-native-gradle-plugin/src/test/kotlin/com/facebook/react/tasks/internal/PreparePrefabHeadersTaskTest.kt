...
class PreparePrefabHeadersTaskTest {
  ...
  fun preparePrefabHeadersTask_withBoostHeaders_filtersThemCorrectly() {
    ...
    File(tempFolder.root, "boost/boost/preprocessor/detail/preprocessor_detail.hpp").createNewPathAndFile()
    ...
    val task = createTestTask<PreparePrefabHeadersTask>(project = project) {
      it.outputDir.set(outputDir)
      it.input.set(
        listOf(PrefabPreprocessingEntry("sample_library", "boost/" to ""))
      )
    }
    ...
    assertTrue(File(outputDir, "sample_library/boost/preprocessor/detail/preprocessor_detail.hpp").exists())
